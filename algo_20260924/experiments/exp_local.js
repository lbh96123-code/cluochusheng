...
const CAPK = [1, 3, 1];
let TEMP = 1; const setTemp = t => { TEMP = t; };   /* 实验: 推演抽样温度(>1 摊平) */
  ...
  while (!F.simDone(sims[0])) {
    ...
    const sampleChunk0 = (c, L) => { const pb = PBUF[c & 1], r0 = c * BFIX, m = Math.min(BFIX, n - r0);
      ...
      for (let r = 0; r < m; r++) { const o6 = r * 60; let mx = -Infinity; for (let i = 0; i < 60; i++) if (pb.legal[o6 + i] && L[o6 + i] > mx) mx = L[o6 + i];
        let z = 0; for (let i = 0; i < 60; i++) { if (pb.legal[o6 + i]) z += Math.exp((L[o6 + i] - mx) / TEMP); cum[o6 + i] = z; } Z[r] = z; }
...
/* 实验: 通用强制推演。forced[b] = [[k, item], ...] (k = 起点之后第几步, 该步不抽样直接落 item; 落子人按顺序表)。
   在落子之前, 这一局的所有待强制 item 不许被别人拿。去重键带上"还没落的强制件"。返回 v(行动方 me 视角胜率)。 */
async function rolloutsG(sim0, own0, me, forced, seed, o = {}) {
  const B = forced.length; if (!B) return { v: new Float64Array(0), tot: 0, uniq: 0 };
  const rngs = Array.from({ length: B }, (_, b) => simRng(seed, b));
  const sims = new Array(B), owns = new Array(B);
  for (let b = 0; b < B; b++) { sims[b] = F.cloneSim(sim0); owns[b] = Int8Array.from(own0); }
  const fmap = forced.map(list => new Map(list.map(([k, it]) => [k, it])));
  let k = 0, tot = 0, uniq = 0; const tAbs0 = o.t0 || 0;
  if (!PBUF) PBUF = [mkBuf(BFIX), mkBuf(BFIX)];
  while (!F.simDone(sims[0])) {
    const seat = sims[0].order[sims[0].step];
    const samp = [];
    for (let b = 0; b < B; b++) { const it = fmap[b].get(k); if (it !== undefined) { F.applySim(sims[b], it); owns[b][it] = seat; fmap[b].delete(k); } else samp.push(b); }
    if (samp.length) {
      const idx = new Map(), rep = [], map = new Int32Array(B);
      for (const b of samp) { const pend = [...fmap[b].values()].sort((x, y) => x - y).join(","); const key = ownKey(owns[b], pend); let r = idx.get(key);
        if (r === undefined) { r = rep.length; idx.set(key, r); rep.push(b); } map[b] = r; }
      const n = rep.length; tot += samp.length; uniq += n;
      const U = new Float64Array(B); for (const b of samp) U[b] = rngs[b]();
      for (let r0 = 0; r0 < n; r0 += BFIX) {
        const m = Math.min(BFIX, n - r0), pb = PBUF[0];
        encodeRange(rep.slice(r0, r0 + m).map(b => sims[b]), rep.slice(r0, r0 + m).map(b => owns[b]), Math.min(49, tAbs0 + k), pb, 0);
        for (let r = 0; r < m; r++) for (const it of fmap[rep[r0 + r]].values()) pb.legal[r * 60 + it] = 0;
        for (const key in W_OF) { const w = W_OF[key]; for (let r = m; r < BFIX; r++) pb[key].copyWithin(r * w, 0, w); }
        const L = (await sess.run(feedsOf(pb, BFIX))).logits.data; ST.calls++;
        const cum = new Float64Array(m * 60), Z = new Float64Array(m);
        for (let r = 0; r < m; r++) { const o6 = r * 60; let mx = -Infinity; for (let i = 0; i < 60; i++) if (pb.legal[o6 + i] && L[o6 + i] > mx) mx = L[o6 + i];
          let z = 0; for (let i = 0; i < 60; i++) { if (pb.legal[o6 + i]) z += Math.exp((L[o6 + i] - mx) / TEMP); cum[o6 + i] = z; } Z[r] = z; }
        for (const b of samp) { const r = map[b] - r0; if (r < 0 || r >= m) continue; const o6 = r * 60;
          if (!(Z[r] > 0)) { F.passSim(sims[b]); o.passes = (o.passes || 0) + 1; continue; }
          const u = U[b] * Z[r]; let pick = -1; for (let i = 0; i < 60; i++) if (pb.legal[o6 + i] && cum[o6 + i] >= u) { pick = i; break; }
          if (pick < 0) { F.passSim(sims[b]); o.passes = (o.passes || 0) + 1; continue; }
          F.applySim(sims[b], pick); owns[b][pick] = seat; }
      }
    }
    k++;
...
/* 实验(MCTS 叶子评估): 从多个起点各推一局到满编; 起点必须处在同一手(step 相同)。返回左队胜率。 */
async function rolloutsMulti(simsIn, ownsIn, seed, o = {}) {
  const B = simsIn.length; if (!B) return new Float64Array(0);
  const rngs = Array.from({ length: B }, (_, b) => simRng(seed, b));
  const sims = simsIn.map(x => F.cloneSim(x)), owns = ownsIn.map(x => Int8Array.from(x));
  let k = 0; const tAbs0 = o.t0 || 0;
  if (!PBUF) PBUF = [mkBuf(BFIX), mkBuf(BFIX)];
  while (!F.simDone(sims[0])) {
    const seat = sims[0].order[sims[0].step];
    const idx = new Map(), rep = [], map = new Int32Array(B);
    for (let b = 0; b < B; b++) { const key = ownKey(owns[b]); let r = idx.get(key); if (r === undefined) { r = rep.length; idx.set(key, r); rep.push(b); } map[b] = r; }
    const n = rep.length, U = new Float64Array(B); for (let b = 0; b < B; b++) U[b] = rngs[b]();
    for (let r0 = 0; r0 < n; r0 += BFIX) {
      const m = Math.min(BFIX, n - r0), pb = PBUF[0];
      encodeRange(rep.slice(r0, r0 + m).map(b => sims[b]), rep.slice(r0, r0 + m).map(b => owns[b]), Math.min(49, tAbs0 + k), pb, 0);
      for (const key in W_OF) { const w = W_OF[key]; for (let r = m; r < BFIX; r++) pb[key].copyWithin(r * w, 0, w); }
      ...
      for (let b = 0; b < B; b++) { const r = map[b] - r0; if (r < 0 || r >= m) continue; const o6 = r * 60;
        if (!(Z[r] > 0)) { F.passSim(sims[b]); continue; }
        const u = U[b] * Z[r]; let pick = -1; for (let i = 0; i < 60; i++) if (pb.legal[o6 + i] && cum[o6 + i] >= u) { pick = i; break; }
        if (pick < 0) { F.passSim(sims[b]); continue; } F.applySim(sims[b], pick); owns[b][pick] = seat; }
    }
    k++;
  }
  const v = new Float64Array(B); for (let b = 0; b < B; b++) v[b] = 1 / (1 + Math.exp(-sims[b].z)); return v;
}
/* 实验: 一批局面(可以处在不同手)的网络出手概率, 合法件上 softmax */
async function netProbsBatch(sims, owns, ts) {
  const out = []; if (!PBUF) PBUF = [mkBuf(BFIX), mkBuf(BFIX)];
  for (let r0 = 0; r0 < sims.length; r0 += BFIX) {
    const m = Math.min(BFIX, sims.length - r0), pb = PBUF[1];
    for (let r = 0; r < m; r++) encodeRange([sims[r0 + r]], [owns[r0 + r]], Math.min(49, ts[r0 + r]), pb, r);
    for (const key in W_OF) { const w = W_OF[key]; for (let r = m; r < BFIX; r++) pb[key].copyWithin(r * w, 0, w); }
    const L = (await sess.run(feedsOf(pb, BFIX))).logits.data;
    for (let r = 0; r < m; r++) { const o6 = r * 60, p = new Float64Array(60); let mx = -Infinity; for (let i = 0; i < 60; i++) if (pb.legal[o6 + i] && L[o6 + i] > mx) mx = L[o6 + i];
      let Z = 0; for (let i = 0; i < 60; i++) if (pb.legal[o6 + i]) { p[i] = Math.exp(L[o6 + i] - mx); Z += p[i]; } if (Z > 0) for (let i = 0; i < 60; i++) p[i] /= Z; out.push(p); }
  }
  return out;
}

/* 实验(用户 09-24 的"共享沙盘"): 多轮推演, 所有推演局共用一张表 T[局面][件] = {N, W(落子方那边赢)}。
   前 D 手之内, 轮到谁谁就在网络概率上按沙盘调整: p ∝ p_net·exp(β·(Q(a) − Q̄)), 再以 ε 均匀探索"网络前 K ∪ 一步加分前 K"。
   第 0 手强制我拿候选 cands[b]; 返回每一轮每个候选的平均胜率(我方视角) + 沙盘(给诊断用)。 */
async function rolloutsShared(sim0, own0, me, cands, o = {}) {
  const I = o.iters || 4, R = o.R || 16, D = o.D || 10, beta = o.beta || 20, eps = o.eps != null ? o.eps : 0.2, K = o.K || 4, nmin = o.nmin || 3;
  const T = new Map(), hist = []; let live = cands.slice(), shareTop = [];
  for (let it = 0; it < I; it++) {
    /* 第 1 轮全部候选×R; 之后只留前 keep 名、每个 R2 局。shareTop = 我这边目前前 S 名: 共享给所有人, 加进他们的探索范围 */
    if (it > 0) { const h = hist[it - 1], s0 = [...h].sort((a, b) => b[1] - a[1]); shareTop = s0.slice(0, o.share || 6).map(x => x[0]); if (it === 1 && o.keep) live = s0.slice(0, o.keep).map(x => x[0]); }
    const RR = it > 0 && o.R2 ? o.R2 : R;
    const cs = []; for (const c of live) for (let r = 0; r < RR; r++) cs.push(c);
    const B = cs.length, rngs = Array.from({ length: B }, (_, b) => simRng((o.seed || 1) * 131 + it, b));
    const sims = new Array(B), owns = new Array(B), trace = Array.from({ length: B }, () => []);
    for (let b = 0; b < B; b++) { sims[b] = F.cloneSim(sim0); owns[b] = Int8Array.from(own0); F.applySim(sims[b], cs[b]); owns[b][cs[b]] = me; }
    let k = 1; if (!PBUF) PBUF = [mkBuf(BFIX), mkBuf(BFIX)];
    while (!F.simDone(sims[0])) {
      const seat = sims[0].order[sims[0].step], sgS = seat < 5 ? 1 : -1, idx = new Map(), rep = [], map = new Int32Array(B), keys = new Array(B);
      for (let b = 0; b < B; b++) { const key = ownKey(owns[b]); keys[b] = key; let r = idx.get(key); if (r === undefined) { r = rep.length; idx.set(key, r); rep.push(b); } map[b] = r; }
      const U = new Float64Array(B), U2 = new Float64Array(B); for (let b = 0; b < B; b++) { U[b] = rngs[b](); U2[b] = rngs[b](); }
      for (let r0 = 0; r0 < rep.length; r0 += BFIX) {
        const m = Math.min(BFIX, rep.length - r0), pb = PBUF[0];
        encodeRange(rep.slice(r0, r0 + m).map(b => sims[b]), rep.slice(r0, r0 + m).map(b => owns[b]), Math.min(49, (o.t0 || 0) + k), pb, 0);
        for (const kk in W_OF) { const w = W_OF[kk]; for (let r = m; r < BFIX; r++) pb[kk].copyWithin(r * w, 0, w); }
        const L = (await sess.run(feedsOf(pb, BFIX))).logits.data; ST.calls++;
        const P = new Array(m);   // 每个代表局面的采样分布(同一局面共用)
        for (let r = 0; r < m; r++) { const o6 = r * 60, b0 = rep[r0 + r], legal = []; let mx = -Infinity;
          for (let i = 0; i < 60; i++) if (pb.legal[o6 + i]) { legal.push(i); if (L[o6 + i] > mx) mx = L[o6 + i]; }
          if (!legal.length) { P[r] = null; continue; }
          const w = legal.map(i => Math.exp(L[o6 + i] - mx)); let Z = w.reduce((a, x) => a + x, 0); for (let q = 0; q < w.length; q++) w[q] /= Z;
          let expl = null;
          if (k <= D) { const st = T.get(keys[b0]);
            if (st) { let qb = 0, nq = 0; for (const [a, e] of st) if (e.N >= nmin) { qb += e.W / e.N; nq++; } qb = nq ? qb / nq : 0;
              if (o.mix) {   /* 混合: (1-λ)·网络 + λ·softmax(β·Q) 只在沙盘里试够 nmin 次的件上; 沙盘证据直接主导, 不被网络的 ~0 先验压住 */
                const qs = legal.map(i => { const e = st.get(i); return e && e.N >= nmin ? e.W / e.N : null; }), have = qs.filter(x => x != null);
                if (have.length >= 2) { const mq = Math.max(...have), sw = qs.map(x => x == null ? 0 : Math.exp(beta * (x - mq))), sz = sw.reduce((a, x) => a + x, 0);
                  for (let q = 0; q < w.length; q++) w[q] = (1 - o.mix) * w[q] + o.mix * sw[q] / sz; } }
              else { for (let q = 0; q < legal.length; q++) { const e = st.get(legal[q]); if (e && e.N >= nmin) w[q] *= Math.exp(beta * (e.W / e.N - qb)); }
                Z = w.reduce((a, x) => a + x, 0); for (let q = 0; q < w.length; q++) w[q] /= Z; } }
            const byNet = legal.map((i, q) => [i, L[o6 + i]]).sort((a, b) => b[1] - a[1]).slice(0, K).map(x => x[0]);
            const byGain = legal.map(i => [i, sgS * F.deltaOf(sims[b0], i)]).sort((a, b) => b[1] - a[1]).slice(0, K).map(x => x[0]);
            expl = [...new Set(byNet.concat(byGain, o.share ? shareTop.filter(i => legal.includes(i)) : []))]; }
          P[r] = { legal, w, expl }; }
        for (let b = 0; b < B; b++) { const r = map[b] - r0; if (r < 0 || r >= m) continue; const pr = P[r]; if (!pr) { F.passSim(sims[b]); continue; }
          let pick;
          if (pr.expl && U2[b] < eps) pick = pr.expl[Math.floor(U[b] * pr.expl.length) % pr.expl.length];
          else { let u = U[b]; pick = pr.legal[pr.legal.length - 1]; for (let q = 0; q < pr.legal.length; q++) { u -= pr.w[q]; if (u <= 0) { pick = pr.legal[q]; break; } } }
          if (k <= D) trace[b].push([keys[b], pick, seat]);
          F.applySim(sims[b], pick); owns[b][pick] = seat; }
      }
      k++;
    }
    const vL = sims.map(s => 1 / (1 + Math.exp(-s.z)));
    for (let b = 0; b < B; b++) for (const [key, a, seat] of trace[b]) { let st = T.get(key); if (!st) T.set(key, st = new Map()); let e = st.get(a); if (!e) st.set(a, e = { N: 0, W: 0 }); e.N++; e.W += seat < 5 ? vL[b] : 1 - vL[b]; }
    const mine = new Map(live.map(c => [c, []])); cs.forEach((c, b) => mine.get(c).push(me < 5 ? vL[b] : 1 - vL[b]));
    hist.push(new Map([...mine].map(([c, a]) => [c, a.reduce((x, y) => x + y, 0) / a.length])));
  }
  return { hist, T };
}
module.exports = { rolloutsShared, rolloutsMulti, netProbsBatch, setTemp, rolloutsG,  init, decide, decideMT, ownerOf, encodeRange, views, rng, logits, statReset, statOut, rollouts, screen, final, refine, myCands, LAST, info: () => INFO, simRng, ownKey, mkBuf };
