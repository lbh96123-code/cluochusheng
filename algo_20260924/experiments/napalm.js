...
const g = JSON.parse(fs.readFileSync(__dirname + "/napgame.json")), pool = AR.poolOf(g), NM = ["L1","L2","L3","L4","L5","R1","R2","R3","R4","R5"];
...
(async () => { await C.init(__dirname + "/data/netB300.onnx", "gpu", 128);
  ...
  const track = ["batrider_sticky_napalm", "npc_dota_hero_phoenix", "phoenix_supernova", "npc_dota_hero_batrider"], ti = track.map(k => sim0.items.findIndex(x => x.key === k));
  ...
  for (let j = 0; j < 50; j++) { const me = AR.FULL[j], mine = NM[me] === "L3";
