...
TAG = f'comp_K{K}_lam{LAM}_b{BINS}' + ('_sgn' if SIGNED else '') + ('_win' if WITHIN else '') + (f'_s{SEED}' if SEED else '') + os.environ.get('EXT_TAG', '') + ('_ps' if os.environ.get('PATCHSPLIT') == '1' else '')
...
A = rng.normal(0, .01, (3 * K + 2, F)).astype(np.float32)
...
def perf_feat(Ss, syn, sq):
    return np.concatenate([Ss, ts(Ss), tsw(Ss), syn[..., None], sq.sum(2)[..., None]], axis=2)
...
for ep in range(EP):
    ...
    for st in range(0, ntr, 2048):
        ...
        if LAM > 0:
            feat = perf_feat(Ss, syn, sq)
            r = feat @ A + c - PERF[ix].astype(np.float32)
            if WITHIN: r = r - r.mean(1, keepdims=True)
            gp = (2.0 * LAM * r / (B * 10 * F)).astype(np.float32)
            gA += feat.reshape(-1, 3 * K + 2).T @ gp.reshape(-1, F)
            gc += gp.sum((0, 1))
            gf = gp @ A.T
        ...
        t += 1
        for i, (p_, gr) in enumerate(zip(PS, (gw, ge, gA, gc, gth, gdw))):
